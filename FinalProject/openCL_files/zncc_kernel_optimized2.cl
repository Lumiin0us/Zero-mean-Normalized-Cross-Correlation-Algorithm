float calculateMean(int x, int y, int d, int width, int height, int halfWinSize, __global const unsigned char* img)
{
...
float calculateZncc(int x, int y, int d, float mean1, float mean2, int width, int height, int halfWinSize, __global const unsigned char* img1, __global const unsigned char* img2)
{
    int yy_0 = max(0, y - halfWinSize);
    int yy_1 = min(height, y + halfWinSize + 1);
    int xx_0 = max(0, x - halfWinSize);
    ...
    int x, int y, int width, int height, int winSize, int maxDisp)
{
    ...
    float mean1 = calculateMean(x, y, 0, width, height, halfWinSize, leftImage);

    for (int d = 0; d < maxDisp; d++)
    {
        float mean2 = calculateMean(x, y, d, width, height, halfWinSize, rightImage);
        float znccVal = calculateZncc(x, y, d, mean1, mean2, width, height, halfWinSize, leftImage, rightImage);
    ...
    int x, int y, int width, int height, int winSize, int maxDisp)
{
    ...
    float mean1 = calculateMean(x, y, 0, width, height, halfWinSize, rightImage);

    for (int d = 0; d < maxDisp; d++)
    {
        float mean2 = calculateMean(x, y, d, width, height, halfWinSize, leftImage);
        float znccVal = calculateZncc(x, y, d, mean1, mean2, width, height, halfWinSize, rightImage, leftImage);
    ...
    int width, int height, int winSize, int maxDisp, int threshold, int max_search_distance)
{
    ...
    disparityLeft[y * width + x] = calculate_disparity_left(leftImg, rightImg, x, y, width, height, winSize, maxDisp);

    // Calculate disparity with right image as reference
    disparityRight[y * width + x] = calculate_disparity_right(leftImg, rightImg, x, y, width, height, winSize, maxDisp);
